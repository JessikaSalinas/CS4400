...
void eval(char *cmdline) 
{
  ...
  // if not builtin command, then executes it                                                       
  // blocks signal until job is added                                                               
  // forks; unblocks child, then parent & child grouped & executed                                  
  // else, if parent then waits for child                                                           
  //   if bg job then unblock signals, set status & print info                                      
  //   if fg job, then unblock signals, set status & wait for fg   
...
int builtin_cmd(char **argv) 
{
  ...
  // checks for quit command, then exits                                                          
  // else checks for jobs command, then lists jobs                                                 
  // else checks for bg or fg commands, then lists those
  if (!strcmp(cmd, "quit")) {
    exit(0);
  }
  ...
  else if (!strcmp(cmd, "bg") || !strcmp(cmd, "fg")) { /* bg and fg commands */
